...
public class Maledictus_Entity extends IABoss_monster implements IHoldEntity {
    ...
    public boolean hurt(DamageSource source, float damage) {
        ...
        if (range > CMConfig.MaledictusLongRangelimit * CMConfig.MaledictusLongRangelimit && !source.isBypassInvul()) {
            return false;
        }
        if ((this.getAttackState() == 31 || this.getAttackState() == 32 || this.getAttackState() == 33) && !source.isBypassInvul()) {
            return false;
        }
    ...
    public void aiStep() {
        ...
        if (this.getAttackState() == 4) {
            if (this.isOnGround() || !this.getFeetBlockState().getFluidState().isEmpty()) {
    ...
    static class MaledictusChargeGoal extends Goal {
        ...
        public void tick() {
            ...
            if (this.entity.getAttackState() == 13) {
                if (entity.attackTicks == 34 && (entity.isOnGround() || entity.isInLava() || entity.isInWater())) {
                    float speed = -1.7f;
                    float dodgeYaw = (float) Math.toRadians(entity.getYRot() + 90);
    ...
    static class MaledictusChargeState extends InternalStateGoal {
        ...
        public void tick() {
            ...
            if (backsteptick > 0 && entity.attackTicks == backsteptick && (entity.isOnGround() || entity.isInLava() || entity.isInWater())) {
                float speed = -1.7f;
                float dodgeYaw = (float) Math.toRadians(entity.getYRot() + 90);
                Vec3 m = entity.getDeltaMovement().add(speed * Math.cos(dodgeYaw), 0, speed * Math.sin(dodgeYaw));
