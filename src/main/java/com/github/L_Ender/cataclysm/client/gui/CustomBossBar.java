...
public class CustomBossBar {
    ...
    public void renderBossBar(CustomizeGuiOverlayEvent.BossEventProgress event) {
        ...
        drawBar(stack, event.getX() + getBaseOffsetX(), y + getBaseOffsetY(), event.getBossEvent());
        ...
        int i1 = i / 2 - l / 2;
        int j1 = j;
        Minecraft.getInstance().font.draw(stack, component.getVisualOrderText(), i1, j1, 16777215);


        if (hasOverlay()) {
            Minecraft.getInstance().getProfiler().push("CataclysmCustomBossBarOverlay");
            RenderSystem.setShaderTexture(0, getOverlayTexture());
            GuiComponent.blit(stack, event.getX() + getBaseOffsetX() + getOverlayOffsetX(), y + getOverlayOffsetY() + getBaseOffsetY(), 0, 0, getOverlayWidth(), getOverlayHeight(), getOverlayWidth(), getOverlayHeight());
            Minecraft.getInstance().getProfiler().pop();
        }
    ...
    private void drawBar(PoseStack stack, int x, int y, BossEvent event) {
    	RenderSystem.setShaderTexture(0, getBaseTexture());
        GuiComponent.blit(stack, x, y, 0, 0, getProgress(), getBaseHeight(), 256, getBaseTextureHeight());
